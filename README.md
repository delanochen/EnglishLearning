...
管理员可从 `/admin/content-center` 创建和控制内容生产任务。阶段 1 已提供 PostgreSQL 任务/批次/条目模型、来源与许可证、导入原文、审核、质量、重复项、计划、使用统计、标签/主题/署名实体，以及受系统管理员权限保护的任务 API。
...
阶段 1 使用数据库任务队列，不在 Web 请求中执行批量生成。Redis、BullMQ 与 `content-worker` 在阶段 3 接入。
