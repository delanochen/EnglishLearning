...
阶段 2 使用数据库任务队列，不在 Web 请求中执行批量生成。可由 NAS 计划任务在项目目录运行 `docker compose exec -T app /app/scripts/run-content-jobs.sh` 消费队列；Redis、BullMQ 与常驻 `content-worker` 在阶段 3 接入。
