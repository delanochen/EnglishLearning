...
阶段 5 已加入 `/admin/content-import` 合法公开资源导入中心、许可证配置、来源白名单、HTTPS/域名/路径限制、DNS 私网阻断、逐次重定向复核、robots.txt 检查、来源级限速、响应类型与 2MB 大小限制、HTML 清洗、原始/标准化哈希去重和完整署名元数据。任何导入内容只进入原始草稿或审核状态，绝不直接发布。

阶段 6 已加入 `/admin/content-initialization` 初始化向导。首次启动且四类内容库均为空时，系统自动建立 6,661 条可恢复的待处理计划；不会在启动请求中调用 AI。管理员确认预算并输入 `START` 后，任务按 20 条分批交给 Worker，支持进度查看、暂停、恢复、取消及失败重试。再次扫描只补齐缺少库存，不重复建立正在执行的初始化任务。

`v0.11.1` 将 `ContentStatus` 枚举扩展拆成独立 PostgreSQL 迁移，并在容器启动时安全清理旧版已经回滚但被 Prisma 记录的失败尝试，避免 NAS 自动部署停在 `P3018 / 55P04`。
