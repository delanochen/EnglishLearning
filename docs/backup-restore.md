...
系统管理员可在“管理后台 → 运维与备份”点击“立即创建完整备份”。该操作在应用容器内调用参数固定的备份脚本，不挂载 Docker Socket，并记录请求、成功或失败审计日志。预升级、定时和命令行备份则使用隔离的 `backup` 容器；两种入口生成相同格式的备份。
...
管理员后台的“运维与备份”页可以逐项下载备份文件，也可以在输入完整备份名称后二次确认删除。应用容器不会挂载危险的 Docker socket；由 root 备份容器生成的目录会在完成后移交给应用 UID 1001，以便后台安全下载和删除。
