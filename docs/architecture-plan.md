...
项目按照需求中的七个阶段增量交付；下文保留最初架构决策和实施路线，当前实现状态以文首和 README 为准。
...
- 外部语音服务和离线视频渲染仍作为可选扩展；PWA、备份恢复和简易课程时间线已在阶段 6–7 落地。
...
七阶段代码已按上述批次完成。每次 NAS 升级仍需遵循备份、迁移、健康检查和回滚文档，不把本地构建成功等同于生产恢复演练。
