...
export default async function ScenariosPage({searchParams}:{searchParams:Promise<{profile?:string;category?:string}>}){
  ...
  const preferredLevels=selected?.level?vocabularyLevelFallbacks(selected.level):[];
  const lessonsRaw=await db.scenarioLesson.findMany({where:{status:"PUBLISHED",...(query.category?{category:query.category}:{}),...(preferredLevels.length?{level:{in:preferredLevels}}:{})},include:{dialogues:true,vocabulary:{orderBy:{order:"asc"}},progress:selected?{where:{learnerProfileId:selected.id}}:false},orderBy:[{category:"asc"},{title:"asc"}]});
  const rank=new Map(preferredLevels.map((level,index)=>[level,index]));const lessons=lessonsRaw.sort((a,b)=>(rank.get(a.level)??99)-(rank.get(b.level)??99));
