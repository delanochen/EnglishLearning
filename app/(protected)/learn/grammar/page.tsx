...
export default async function GrammarPage() {
  ...
  const preferredLevels=selected?.level?vocabularyLevelFallbacks(selected.level):[];const topicsRaw = await db.grammarTopic.findMany({ where: { status: "PUBLISHED", ...(preferredLevels.length ? { level:{in:preferredLevels} } : {}) }, include: { progress: selected ? { where: { learnerProfileId: selected.id } } : false }, orderBy: { title: "asc" } });const topics=selectClosestLevelItems(topicsRaw,selected?.level,item=>item.level);
