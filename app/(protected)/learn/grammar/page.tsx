...
export default async function GrammarPage() {
  ...
  const topicsRaw = await db.grammarTopic.findMany({ where: { status: "PUBLISHED", ...(selected?.level ? { level:selected.level } : {}) }, include: { progress: selected ? { where: { learnerProfileId: selected.id } } : false }, orderBy: { title: "asc" } });const topics=selectExactLevelItems(topicsRaw,selected?.level,item=>item.level);
