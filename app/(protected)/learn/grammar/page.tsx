...
export default async function GrammarPage() {
  ...
  const preferredLevels=selected?.level?vocabularyLevelFallbacks(selected.level):[];const topicsRaw = await db.grammarTopic.findMany({ where: { status: "PUBLISHED", ...(preferredLevels.length ? { level:{in:preferredLevels} } : {}) }, include: { progress: selected ? { where: { learnerProfileId: selected.id } } : false }, orderBy: { title: "asc" } });const rank=new Map(preferredLevels.map((level,index)=>[level,index]));const topics=topicsRaw.sort((a,b)=>(rank.get(a.level)??99)-(rank.get(b.level)??99));
