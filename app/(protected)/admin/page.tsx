...
export default async function AdminPage() {
  ...
  return <div className="mx-auto max-w-6xl"><p className="text-sm font-bold uppercase tracking-[.2em] text-brand">System admin</p><h1 className="mt-2 text-4xl font-black">管理后台</h1><section className="mt-8 grid gap-4 sm:grid-cols-3"><div className="metric"><span className="text-muted">用户</span><strong className="text-4xl">{users}</strong></div><div className="metric"><span className="text-muted">家庭</span><strong className="text-4xl">{families}</strong></div><div className="metric"><span className="text-muted">系统状态</span><strong className="text-2xl text-brand">正常</strong></div></section><section className="card mt-6"><h2 className="text-xl font-bold">系统配置</h2><div className="mt-4 flex flex-wrap gap-3"><Link href="/admin/ai" className="button-primary inline-block">AI 模型管理</Link><Link href="/admin/operations" className="button-ghost inline-block">运维与备份状态</Link></div></section><section className="card mt-6"><h2 className="text-xl font-bold">最近审计记录</h2><div className="mt-4 divide-y divide-slate-200 dark:divide-slate-700">{audits.map((audit) => <div className="flex justify-between gap-5 py-3 text-sm" key={audit.id}><span>{audit.action} · {audit.resourceType}</span><time className="text-muted">{audit.createdAt.toLocaleString("zh-CN")}</time></div>)}{!audits.length && <p className="py-5 text-muted">暂无操作记录</p>}</div></section></div>;
}
