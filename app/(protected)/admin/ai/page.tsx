...
export default async function AIAdminPage() {
  ...
  return <div className="mx-auto max-w-6xl">
    ...
    <div className="mt-6 space-y-5">{providers.map((provider) => <section className="card" key={provider.id}>
      ...
      <div className="mt-6"><h3 className="font-bold">模型</h3><div className="mt-2 space-y-2">{provider.models.map((model) => <details className="rounded-xl bg-slate-50 p-3 text-sm dark:bg-slate-800" key={model.id}><summary className="cursor-pointer"><strong>{model.displayName}</strong> <span className="text-muted">{model.name} · {model.enabled ? model.status : "DISABLED"} · 优先级 {model.priority}{model.isDefault ? " · 默认" : ""}</span></summary><form action={saveModel} className="mt-4 grid gap-3 md:grid-cols-3"><input type="hidden" name="modelId" value={model.id}/><input type="hidden" name="providerId" value={provider.id}/><label><span className="label">API 模型 ID</span><input className="input" name="name" defaultValue={model.name} required/></label><label><span className="label">显示名称</span><input className="input" name="displayName" defaultValue={model.displayName} required/></label><label><span className="label">Capabilities（逗号分隔）</span><input className="input" name="capabilities" defaultValue={model.capabilities.join(",")}/></label><label><span className="label">Temperature</span><input className="input" type="number" step="0.1" name="temperature" defaultValue={model.temperature}/></label><label><span className="label">Max Tokens</span><input className="input" type="number" name="maxTokens" defaultValue={model.maxTokens}/></label><label><span className="label">优先级</span><input className="input" type="number" name="priority" defaultValue={model.priority}/></label><label className="flex items-center gap-2"><input type="checkbox" name="enabled" defaultChecked={model.enabled}/>启用</label><label className="flex items-center gap-2"><input type="checkbox" name="isDefault" defaultChecked={model.isDefault}/>设为 Provider 默认模型</label><button className="button-primary">保存模型</button></form><form action={removeModel} className="mt-3"><input type="hidden" name="id" value={model.id}/><button className="button-ghost text-red-700">移除模型</button></form></details>)}{!provider.models.length && <p className="text-sm text-muted">尚未添加模型；添加后才能测试连接。</p>}</div>
        <form action={addModel} className="mt-4 grid gap-3 md:grid-cols-3"><input type="hidden" name="providerId" value={provider.id}/><input className="input mt-0" name="name" placeholder="模型名称/API ID" required/><input className="input mt-0" name="displayName" placeholder="显示名称" required/><input className="input mt-0" name="capabilities" defaultValue="chat,structured-output"/><input className="input mt-0" type="number" step="0.1" name="temperature" defaultValue="0.7"/><input className="input mt-0" type="number" name="maxTokens" defaultValue="2048"/><input className="input mt-0" type="number" name="priority" defaultValue="100"/><label className="flex items-center gap-2"><input type="checkbox" name="isDefault"/>设为默认</label><button className="button-primary">添加模型</button></form>
      </div>
    </section>)}</div>
    ...
    <section className="card mt-6"><h2 className="text-xl font-bold">AI 用途路由</h2><p className="mt-1 text-sm text-muted">为每个用途添加一个或多个模型；数字越小越优先，失败时按顺序切换。</p>
      ...
      <div className="mt-5 space-y-2">{routes.map((route) => <div className="rounded-xl border border-slate-200 p-3 dark:border-slate-700" key={route.id}><strong>{purposeLabels[route.purpose]}</strong><div className="mt-2 flex flex-wrap gap-2">{route.models.map((item) => <form action={removeRouteModel} className="flex items-center gap-2 rounded-lg bg-slate-100 px-3 py-2 text-sm dark:bg-slate-800" key={item.id}><input type="hidden" name="id" value={item.id}/><span>{item.priority}: {item.model.provider.name}/{item.model.displayName}</span><button className="text-red-700" title="从此用途移除">×</button></form>)}{!route.models.length && <span className="text-sm text-muted">无模型</span>}</div></div>)}</div>
    </section>
  </div>;
}
