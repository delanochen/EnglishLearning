...
export default async function FamilyDashboardPage() {
  ...
  const t = english ? { title: "Family learning dashboard", goal: "Shared goal", noGoal: "Not set", weeklyChallenge:"Family weekly challenge",challengeDone:"Challenge completed — everyone contributed!",leaderboard: "Family leaderboard enabled", member: "Member", time: "Today / week", progress: "Weekly change", compared: "vs previous 7 days", streak: "Streak", tasks: "Tasks", mastered: "Words mastered", accuracy: "Accuracy", recent: "Recent learning", advice: "Alert / next-week advice", minutes: "min", days: "days", none: "None", inactive: "No activity for 7 days", fallbackLow: "Reduce the task load and prioritize weak areas", fallbackGood: "Keep the current pace", sharedGoal: "Family shared goal", goalPlaceholder: "For example: study five days per week as a family", enableLeaderboard: "Enable leaderboard", save: "Save settings" } : { title: "家庭学习仪表盘", goal: "共同目标", noGoal: "尚未设置",weeklyChallenge:"家庭本周共同挑战",challengeDone:"挑战完成，全家共同进步！", leaderboard: "家庭排行榜已开启", member: "成员", time: "今日 / 本周", progress: "本周进步", compared: "较前 7 天", streak: "连续", tasks: "任务", mastered: "掌握单词", accuracy: "正确率", recent: "最近内容", advice: "提醒 / 下周建议", minutes: "分钟", days: "天", none: "暂无", inactive: "7 天无活动", fallbackLow: "减少任务量并优先完成薄弱项", fallbackGood: "保持当前节奏", sharedGoal: "家庭共同目标", goalPlaceholder: "例如：全家每周学习 5 天", enableLeaderboard: "开启排行榜", save: "保存设置" };
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <div className="mt-7 space-y-6">{families.map((family) => {
      ...
      const weeklyMinutes=rows.reduce((sum,row)=>sum+row.minutes,0);const weeklyTarget=Math.max(60,family.members.reduce((sum,member)=>sum+(member.learnerProfile?.dailyMinutes??20)*5,0));const weeklyPercent=Math.min(100,Math.round(weeklyMinutes/weeklyTarget*100));
      ...
      return <section className="card" key={family.id}>
        ...
        <div className="mt-5 rounded-2xl bg-brand/10 p-5"><div className="flex justify-between gap-4"><strong>{t.weeklyChallenge}</strong><span>{weeklyMinutes}/{weeklyTarget} {t.minutes}</span></div><div className="mt-3 h-3 overflow-hidden rounded-full bg-white/80 dark:bg-slate-800"><div className="h-full rounded-full bg-brand" style={{width:`${weeklyPercent}%`}}/></div>{weeklyPercent>=100&&<p className="mt-2 font-bold text-brand">🏅 {t.challengeDone}</p>}</div>
