...
export async function achievementMetrics(learnerProfileId:string){
  const [profile,tasks,minutes,vocabulary,reading,speaking,scenarios,modules,highScores,games]=await Promise.all([
    ...
    db.vocabularyGameSession.findMany({where:{learnerProfileId},select:{mode:true,totalQuestions:true,correctAnswers:true,maxStreak:true}}),
  ]);
  return {TASKS:tasks,STREAK:profile.studyStreak?.currentDays??0,XP:profile.totalXp,MINUTES:Math.floor((minutes._sum.durationSeconds??0)/60),VOCABULARY:vocabulary,READING:reading,SPEAKING_MINUTES:Math.floor((speaking._sum.durationSeconds??0)/60),SCENARIOS:scenarios,MODULES:modules.length,HIGH_SCORE:highScores,GAME_STREAK:Math.max(0,...games.map(game=>game.maxStreak)),PERFECT_WEEKLY:games.filter(game=>game.mode==="WEEKLY"&&game.correctAnswers===game.totalQuestions).length} as Record<string,number>;
}
