...
export async function submitPlacementTest(formData:FormData){
  ...
  await db.$transaction(async tx=>{
    ...
    const plan=await tx.learningPlan.create({data:{learnerProfileId:profileId,type:"WEEKLY",periodStart,periodEnd,goals:learner.goals.length?learner.goals:["建立稳定英语学习习惯"],generationSource:"PLACEMENT_TEST",adjustmentReason:`根据第 ${version} 次水平测试自动生成`,version:planVersion}});
    ...
    await tx.learningActivity.create({data:{learnerProfileId:profileId,familyId:accessible.familyId,activityType:"PLACEMENT_TEST",module:"ASSESSMENT",score:result.ratio,sourceType:"PlacementTest",sourceId:test.id}});
  });
  redirect(`/placement?profile=${profileId}&completed=${version}`);
}
