...
async function finalizeBatch(batchId:string|null){if(!batchId)return;const batch=await db.contentGenerationBatch.findUnique({where:{id:batchId}});if(batch&&batch.completedItems+batch.failedItems>=batch.totalItems)await db.contentGenerationBatch.update({where:{id:batchId},data:{status:batch.failedItems>0?"REVIEW_REQUIRED":"DRAFT",finishedAt:new Date()}})}
async function finalizeJob(jobId:string){const job=await db.contentGenerationJob.findUnique({where:{id:jobId}});if(job&&job.completedItems+job.failedItems>=job.totalItems)await db.contentGenerationJob.update({where:{id:jobId},data:{status:job.failedItems>0?"REVIEW_REQUIRED":"DRAFT",currentProgress:100,finishedAt:new Date()}})}
