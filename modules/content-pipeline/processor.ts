...
export async function processNextContentItem(jobId:string){
  ...
  try{
    ...
    const configuration=job.configuration&&typeof job.configuration==="object"&&!Array.isArray(job.configuration)?job.configuration as Record<string,unknown>:{};
    const autoPublish=configuration.autoPublish===true||configuration.initialization===true;
...
async function publishGeneratedContent(reference:{contentType:string;contentId:string},actorUserId:string|null){
  const data={status:"PUBLISHED" as const,reviewStatus:"APPROVED" as const,publishedAt:new Date()};
  await db.$transaction(async tx=>{
    if(reference.contentType==="Vocabulary")await tx.vocabulary.update({where:{id:reference.contentId},data});
    else if(reference.contentType==="ReadingArticle")await tx.readingArticle.update({where:{id:reference.contentId},data});
    else if(reference.contentType==="GrammarTopic")await tx.grammarTopic.update({where:{id:reference.contentId},data});
    else if(reference.contentType==="ScenarioLesson")await tx.scenarioLesson.update({where:{id:reference.contentId},data});
    else throw new Error(`UNSUPPORTED_CONTENT_REFERENCE:${reference.contentType}`);
    await tx.auditLog.create({data:{actorUserId,action:"CONTENT_AUTO_PUBLISHED",resourceType:reference.contentType,resourceId:reference.contentId,metadata:{reason:"INITIALIZATION_QUALITY_PASSED"}}});
  });
}
...
async function finalizeBatch(batchId:string|null){if(!batchId)return;const batch=await db.contentGenerationBatch.findUnique({where:{id:batchId}});if(batch&&batch.completedItems+batch.failedItems>=batch.totalItems){const reviews=await db.contentGenerationItem.count({where:{batchId,status:"REVIEW_REQUIRED"}});const published=await db.contentGenerationItem.count({where:{batchId,status:"PUBLISHED"}});await db.contentGenerationBatch.update({where:{id:batchId},data:{status:batch.failedItems>0||reviews>0?"REVIEW_REQUIRED":published===batch.totalItems?"PUBLISHED":"DRAFT",finishedAt:new Date()}})}}
async function finalizeJob(jobId:string){const job=await db.contentGenerationJob.findUnique({where:{id:jobId}});if(job&&job.completedItems+job.failedItems>=job.totalItems){const reviews=await db.contentGenerationItem.count({where:{jobId,status:"REVIEW_REQUIRED"}});const published=await db.contentGenerationItem.count({where:{jobId,status:"PUBLISHED"}});await db.contentGenerationJob.update({where:{id:jobId},data:{status:job.failedItems>0||reviews>0?"REVIEW_REQUIRED":published===job.totalItems?"PUBLISHED":"DRAFT",currentProgress:100,finishedAt:new Date()}})}}
