...
const question = (i: number) => ({ type: "MULTIPLE_CHOICE", prompt: `distinct question ${i + 1}`, options: ["complete correct option text", "distractor one", "distractor two"], answerKey: "complete correct option text", explanation: "string" });
const schemaExample: Record<SupportedGenerationType, unknown> = {
  ...
  SCENARIO_GENERATION: { category: "string", title: "string", intro: "A practical introduction containing at least eighty characters for this complete American-life scenario lesson.", level: "A2", cultureTips: ["tip one", "tip two", "tip three"], misunderstandings: ["example one", "example two"], naturalExpressions: ["phrase one", "phrase two", "phrase three", "phrase four"], dialogues: Array.from({ length: 16 }, (_, i) => ({ speaker: i % 2 ? "Staff" : "Customer", roleName: i % 2 ? "工作人员" : "顾客", textEn: `complete dialogue line number ${i + 1}`, textZh: `完整对话内容 ${i + 1}`, cameraCue: "string" })), exercises: Array.from({ length: 6 }, (_, i) => question(i)), listeningScript: "A practical listening script of at least 100 characters with a complete interaction between the customer and staff, including a problem, clarification, and resolution.", rolePlayPrompt: "A detailed role-play prompt of at least twenty characters.", vocabulary: Array.from({ length: 6 }, (_, i) => ({ word: `word${i + 1}`, meaningZh: `释义${i + 1}`, example: `example ${i + 1}` })) },
};
