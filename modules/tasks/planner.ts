...
export type LearnerTaskContext={profileId?:string;level?:string|null;interests?:string[];goals?:string[]};
...
export function planDailyTasks(dailyMinutes:number,rotationDay=0,weakAreas:string[]=[],context:LearnerTaskContext={}):PlannedTask[]{
  ...
  const vocabulary=Math.max(6,Math.round(minutes*.3));const mainMinutes=Math.max(8,Math.round(minutes*.4));const remaining=minutes-vocabulary-mainMinutes;const weak=weakAreas.join(" ").toLowerCase();const offset=stableSeed(context.profileId)%modules.length;
  const weakModule=modules.find(module=>weakAreaTerms[module.taskType].some(term=>weak.includes(term)));
  const preferred=weakModule??modules[(rotationDay+offset)%modules.length];const secondary=modules[(rotationDay+offset+3)%modules.length];
